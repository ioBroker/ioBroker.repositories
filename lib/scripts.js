...
function formatMaintainer(entry, authors) {
    ...
    if (typeof entry === 'object') {
        ...
        if (authors[name]) name += '(' +   authors[name] + ')';
            ...
            getDiscovery(discovery => {
                serial(tasks, results => {
                    ...
                    text += '<tr>' +
                        ...
                        '<th class="header-discover">Discover</th>' +
                        '<th class="header-maintainer">Maintainer</th>' +
                        '<th class="header-created-on">Created</th>' +
                        '<th class="header-version">Versions</th>' +
                        '</tr>';
                    let i = 1;
                    ...
                    for (let adapter in latest) {
                        ...
                        try {
                            ...
                            text += '<td><a href="' + aItem.link + '" target="_blank">' + adapter + '</a></td>';
                            ...
                            // Description
                            aItem.desc = (git ? (git.desc.en || git.desc) : '');
                            text += '<td>' + (aItem.desc || '-') + '</td>';
                            ...
                            text += '<td><span style="' + (aItem.typeError ? 'color: red' : '') + '" title="' +
                                aItem.typeTitle
                                + '">' + aItem.type + '</span></td>';
                            ...
                            if (npm && npm.info.time.created) {
                                ...
                                let sDate =  ('0' + (date.getDate())).slice(-2) + '.' +
                                    ...
                                    date.getFullYear();
                                text += '<td style="' + ((now.getTime() - date.getTime()) < 3600000 * 24 * 30 ? 'color: green; font-weight: bold' : '') + '">' + sDate + '</td>';
                            } else {
                                text += '<td>-</td>';
                            }
                            ...
                            aItem.versions = {
                                ...
                                stableDate: npm && npm.info.time[stable[adapter].version]
                            };

                            text += '<td><table class="table-version">' +
                                '<tr><td>github:</td><td>' + (aItem.versions.github || '-.-.-') + (aItem.githubDate ? ' ' + getInterval(aItem.githubDate) : '') + '</td></tr>' +
                                '<tr><td>latest:</td><td>' + (aItem.versions.latest || '-.-.-') + (aItem.latestDate ? ' ' + getInterval(aItem.latestDate) : '') + '</td></tr>' +
                                '<tr><td>stable:</td><td>' + (aItem.versions.stable || '-.-.-') + (aItem.stableDate ? ' ' + getInterval(aItem.stableDate) : '') + '</td></tr></table></td>';
