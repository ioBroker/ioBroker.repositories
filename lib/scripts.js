...
function updatePublished(name, callback, _sources) {
    let cmd = 'npm show ' + tools.appName.toLowerCase() + '.' + name + ' time.created';
    let exec = require('child_process').exec;
    ...
    exec(cmd, function(error, stdout, stderr) {
        if (stderr) {
            ...
            _sources[name].published = stdout.replace('\n', '');
            console.log(tools.appName.toLowerCase() + '.' + name + ' - ' + _sources[name].published);
            callback(null, _sources, name, _sources[name].version);
        }
    });
}

function updatePublishes(callback, _sources, _errors) {
    if (!_sources) {
        _sources = require(__dirname + '/../sources-dist.json');
        // rebuild order
        _sources = sortRepo(_sources);
    }
    ...
    for (let name in _sources) {
        if (!_sources.hasOwnProperty(name)) continue;
        if (!_sources[name].published && (!_errors || _errors.indexOf(name) === -1)) {
            someRequested = true;
            updatePublished(name, function (err, ignore, _name) {
                if (err) {
                    _errors = _errors || [];
                    _errors.push(_name);
                }
                setTimeout(updatePublishes, 0, callback, _sources, _errors);
            }, _sources);
            break;
        }
    }
