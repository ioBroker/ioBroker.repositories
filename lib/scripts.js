    ...
    return function (callback) {
        ...
        axios(latest[adapter].meta)
            .then(data => {
                try {
                    const info = data.data;
                    callback({ adapter, license: info.common.license, version: info.common.version, desc: info.common.desc, git: true, info });
                } catch (e) {
        ...
        .then(discovery => {
            ...
            return new Promise(resolve =>
                serial(tasks, results => {
                    ...
                    now = `${(`0${now.getDate()}`).slice(-2)}.${('0' + (now.getMonth() + 1)).slice(-2)} ${('0' + now.getHours()).slice(-2)}:${('0' + now.getMinutes()).slice(-2)}`;
