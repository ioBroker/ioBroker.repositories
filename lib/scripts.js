        ...
        request(url, (error, state, body) => {
            try {
                ...
                if (!info || info.error || !info.repository || typeof info.repository.url !== 'string') {
                    reject(`Could not find git repo for ${adapterName}!`);
                }
                /** @type {string} */
        ...
        request(metaUrl, (error, state, body) => {
            try {
                ...
                if (!info || info.error || !info.common || typeof info.common.extIcon !== 'string') {
                    reject(`Could not parse adapter meta at ${metaUrl}`);
                }
                /** @type {string} */
...
function init(callback) {
    const latest = require('../sources-dist');
    const stable = require('../sources-dist-stable');
...
async function addToLatest(adapterName, type) {
    ...
    const sources = require('../sources-dist.json');
    if (adapterName in sources) throw new Error(`${adapterName} is already in latest!`);
    sources[adapterName] = {
        meta: metaUrl,
        icon: iconUrl,
        type
...
function removeDates() {
    ...
    Object.keys(sources).forEach(name => {
        delete sources[name].published;
        delete sources[name].versionDate;
    });
    fs.writeFileSync(__dirname + '/../sources-dist-stable.json', JSON.stringify(sortRepo(sources), null, 2));
...
if (module.exports && module.parent) {
    ...
    if (process.argv.indexOf('--init') !== -1) {
        init(() => process.exit());
    } else if (process.argv.indexOf('--nodates') !== -1) {
        removeDates();
