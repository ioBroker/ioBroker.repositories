...
function updateVersion(name, callback, _sources) {
    const cmd = 'npm show ' + tools.appName.toLowerCase() + '.' + name + ' version';
    const exec = require('child_process').exec;
	
    // If update for new adapter => read info about it from latest
    if (!_sources[name]) {
        const latest = JSON.parse(fs.readFileSync(__dirname + '/../sources-dist.json').toString());
...
function updateVersions(callback, _sources) {
    if (!_sources) {
        ...
        const newSources = require(__dirname + '/../sources-dist.json');
        const names = [];
        for (const n in newSources) {
...
function updatePublished(name, callback, _latest, _stable) {
    const cmd = 'npm show ' + tools.appName.toLowerCase() + '.' + name + ' time';
    const exec = require('child_process').exec;
...
if (module.exports && module.parent) {
    ...
    module.exports.update = function (name, callback, _sources) {
        updateVersion(name, function (err, sources) {
            ...
            const sources_ = JSON.parse(fs.readFileSync(__dirname + '/../sources-dist.json').toString());
