...
async function checkNpm(adapter) {
    ...
    if (!response?.data) {
        ...
        if (!body.includes('href="/~bluefox"') && body.includes('href="/~iobluefox"')) {
            return `Bluefox was not found in the collaborators on NPM!. Please execute in adapter directory: "npm owner add bluefox iobroker.${adapter}"`k
        }
...
async function updateSummaryIssue(adapters) {
    ...
    issues = issues.filter(i => i.state === 'open' && i.title.includes(title));
    if (!issues.length) {
	conesole.log('warning: Could not locate base issue "${title}", will create one');
        let body = `# ioBroker adapter checker for npm access\n\n`;
...
//        }
    ...
    console.log(`updateing issue ${issueId}`);
