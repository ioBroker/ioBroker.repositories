...
async function generateIssue(adapter) {
	...
	console.log(`create issue for ioBroker.${adapter.adapter}`);
        ...
        try {
            if (adapter.owner === 'iobroker-community-adapters') {
...
//        }
    ...
    for (const old of oldList) {
        for (const adapter of adapters) {
            if (old.startsWith(`- [ ] ${adapter.owner}/${adapter.adapter}`)) {
                newList.push(old);
                adapter.old=true;
            }
	}
    ...
    for (const adapter of adapters) {
	if (!adapter.old)
	    body += `- [ ] ${adapter.owner}/${adapter.adapter} - [since ${date.toLocaleString('de-DE').split(',')[0]}]\n`
    }
