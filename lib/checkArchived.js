...
async function generateIssue(adapter, stableFile) {
    ...
    if (!issues.length) {
	  ...
	  if ( adapter.archived) {
            body += `Repository ${adapter.owner}/${adapter.adapter} is archived\n`;
	  };
	  if ( adapter.error) {
            body += `Retrieving data from repository ${adapter.owner}/${adapter.adapter} failed\n`;
            body += `${e}\n`;
	  };
	  body += '\n';
	  ...
	  if ( adapter.archived) {
            body += `- [ ] verify that repository https://github.com/${adapter.owner}/${adapter.adapter} is really archived\n`;
	  ...
	  if ( adapter.error) {
            body += `- [ ] check repository ${adapter.owner}\${adapter.adapter}\n`;
