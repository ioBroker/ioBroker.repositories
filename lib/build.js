...
function getStats(cb) {
    if (cb) {
        ...
        return new Promise((resolve, reject) =>
            request('http://iobroker.live/statistics.json', (error, state, body) => {
                if (error || state.statusCode >= 400) {
                    reject(error || state.statusCode >= 400);
                } else {
                    const data = JSON.parse(body);
                    resolve(data.adapters);
                }
            }));
    }
}
...
function getImages(list, destination, callback) {
    if (!list || !list.length) {
        ...
        if (!fs.existsSync(destination + name.name + '.svg')) {
            ...
            request.get({url: `http://img.shields.io/npm/v/iobroker.${name.name}.svg?style=flat-square`, encoding: 'binary'}, (error, response, body) => {
                if (!error && body) {
...
function getLicenses(list, destination, callback) {
    if (!list || !list.length) {
        ...
        const name = list.pop();
        request.get({url: `https://img.shields.io/github/license/${name}.svg?style=flat-square`, encoding: 'binary'}, (error, response, body) => {
            if (!error && body) {
