...
async function doIt() {
    ...
    // Check if PR only changes repository files - if so, skip the review process
    const onlyRepositoryFiles = fileNames.every(fileName => 
        fileName === 'sources-dist.json' || fileName === 'sources-dist-stable.json'
    );

    if (onlyRepositoryFiles && fileNames.length > 0) {
        console.log('PR only changes repository files (sources-dist.json or sources-dist-stable.json). Skipping automated review.');
        return Promise.resolve('No review needed for repository file changes only');
    }
