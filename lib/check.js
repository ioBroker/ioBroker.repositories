'use strict';
const fs = require('fs');
// const compare-versions = require('compare-versions'); // this package is used in checker.js, so do not remove them from package.json
// const json5 = require('json5'); // this package is used in checker.js, so do not remove them from package.json
...
function downloadChecker() {
    return getGithub('https://raw.githubusercontent.com/ioBroker/ioBroker.repochecker/master/index.js')
        .then(body => {
            ...
            fs.writeFileSync(`${__dirname}/package.json`, '{"version": "check"}');
...
function detectChanges(commit) {
     ...
     */
    return getGithub(commit.raw_url, true)
        .then(json => {
