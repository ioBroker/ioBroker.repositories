...
function decorateLine(line) {
    ...
    if (m) {
        line.text = line.text.replace(`travis`, `[travis](https://travis-ci.org/)`);
    }
    ...
    if (m) {
        line.text = line.text.replace(` README.md`, " [README.md](" + line.link + "/blob/master/README.md)");
    }

    m = line.text.match(/ io\-package\.json/);
    if (m) {
        line.text = line.text.replace(` io-package.json`, ` [io-package.json]("${line.link}/blob/master/io-package.json)`);
    }

    m = line.text.match(/ package\.json/);
    if (m) {
        line.text = line.text.replace(` package.json`, ` [package.json]("${line.link}/blob/master/package.json)`);
    }

    m = line.text.match(/ node_modules/);
    if (m) {
        line.text = line.text.replace(` node_modules`, ` [node_modules]("${line.link}/tree/master/node_modules)`);
    }
    ...
    return downloadChecker()
        ...
        .then(results => {
            ...
            results.forEach(data => {
                ...
                comments.push({text: `### [${adapter}](${link})`, link, owner, adapter});
                if (data.context) {
                    someChecked = true;
                    if (data.context.errors && data.context.errors.length) {
                        errorsFound = true;
                        data.context.errors.forEach(err => comments.push({text: ' * ' + err, link, owner, adapter}));
                    } else {
                        comments.push({text: 'No errors found', link, owner, adapter});
                    }
                }
            });
            const promises = [];
            if (!someChecked) {
                comments.push({text: 'No changed adapters found'});
            } else {
                if (errorsFound) {
