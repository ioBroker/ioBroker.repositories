    ...
    for (let i = 0; i < links.length; i++) {
        ...
        if (data.context) {
            ...
            if (data.context.warnings && data.context.warnings.length) {
                ...
                data.context.warnings.filter(warn => warn.startsWith('[W')).forEach(warn => comments.push({text: `- [ ] :eyes: ${warn}`, link, owner, adapter}));
                comments.push({text: `  `, link, owner, adapter});
            }

            if (data.context.warnings && data.context.warnings.length) {
                comments.push({text: `**SUGGESTIONS:**`, link, owner, adapter});
                data.context.warnings.filter(warn => warn.startsWith('[S')).forEach(warn => comments.push({text: `- [ ] :pushpin: ${warn}`, link, owner, adapter}));
                comments.push({text: `  `, link, owner, adapter});
            }
        }
