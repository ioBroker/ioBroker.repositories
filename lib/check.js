...
async function doIt() {
    ...
    for (let i = 0; i < links.length; i++) {
        console.log (``);
        console.log (`cheching ${owner}/${adapter}`);
        ...
        try {
            console.log(`Triggering repository check workflow for ${owner}/${adapter}`);
            triggerRepoCheck(owner, adapter);
        } catch (e) {
            console.error(`Could not trigger repository check workflow for ${owner}/${adapter}: ${e}`);
        }
