...
function getPullRequestNumber() {
    if (process.env.GITHUB_REF) {
        ...
        process.env.GITHUB_REF = event.pull_request ? event.pull_request.number : (event.issue ? event.issue.number : '');
    } else {
...
function doIt() {
    ...
    if (process.env.GITHUB_EVENT_PATH) {
        ...
        if (event.action === 'created' && event.issue) {
            if (!event.issue.body.includes('RE-CHECK!')) {
    ...
    return downloadChecker()
        ...
        .then(results => {
            const comments = [{text: '## Automated adapter checker'}];
            ...
            return getAllComments(prID)
                .then(comments => {
                    const exists = comments.find(comment => comment.body.includes('## Automated adapter checker'));
                    if (exists) {
...
console.log('process.env.GITHUB_TOKEN = ' +  process.env.GITHUB_TOKEN.length);
