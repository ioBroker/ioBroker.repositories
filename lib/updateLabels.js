...
const LABEL_STALE = 'stale';
const LABEL_STALE_CLOSING = 'stale - marked for closing';
...
async function processPr(issue) {
    ...
    // stale PRs get neither wip nor ok - remove any existing ones and stop here
    if (labels.has(LABEL_STALE) || labels.has(LABEL_STALE_CLOSING)) {
        console.log(`    PR is stale - removing '${LABEL_WIP}' and '${LABEL_OK}'`);
