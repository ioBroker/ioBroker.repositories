...
async function doIt() {
    ...
    // Check if PR only changes repository files - if so, skip adding informational comments
    ...
    const onlyRepositoryFiles = fileNames.every(fileName => 
        fileName === 'sources-dist.json' || fileName === 'sources-dist-stable.json'
    ) && fileNames.length > 0;

    if (onlyRepositoryFiles) {
        console.log('PR only changes repository files (sources-dist.json or sources-dist-stable.json). Skipping informational comment.');
        return Promise.resolve('No informational comment needed for repository file changes only');
    }
