 ...
 */
function extractLicense(options) {
    const {ioPackJson, packJson} = options;
    if (ioPackJson.common.license && typeof ioPackJson.common.license) {
        const {license, type, link} = ioPackJson.common.license;
        return { license, type, link };
    }
...
function getInstalledInfo(hostRunningVersion) {
    ...
    const licenseInfo = extractLicense({ packJson: package_, ioPackJson: ioPackage });
...
function _getRepositoryFile(sources, path, callback) {
    if (!sources._helper) {
        ...
        for (const _name in sources) {
            if (!sources.hasOwnProperty(_name)) {
                continue;
            }
            count++;
        }
        ...
        sources._helper.timeout = setTimeout(function () {
            if (sources._helper) {
                ...
                for (const __name in sources) {
                    if (!sources.hasOwnProperty(__name)) {
                        continue;
                    }
...
function sendDiagInfo(obj, callback) {
    axios.post(`http://download.${module.exports.appName}.net/diag.php`, `data=${JSON.stringify(obj)}`, {
