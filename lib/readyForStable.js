...
function generateIssue(adapter, stableFile) {
    ...
    return getGithub(`https://api.github.com/repos/${adapter.owner}/ioBroker.${adapter.adapter}/issues`)
        ...
        .then(json => json.filter(i => i.state === 'open' && i.title.includes('Update stable version in repo')))
        .then(issues => {
            if (!issues.length) {
                // find line count
                ...
                return createIssue(adapter.owner, 'ioBroker.' + adapter.adapter,  {
                    title: TITLE + ' to ' + adapter.latest.version,
                    body
                });
            }
        });
...
let stats;
let master;
console.log('process.env.GITHUB_TOKEN=' + process.env.GITHUB_TOKEN.length + ' - '+ process.env.GITHUB_TOKEN.replace(/.+/g, 'x'));
