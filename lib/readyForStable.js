...
console.log(`    Installs: stable=${item.stable.installs} (${item.stable.percent}%), latest=${item.latest.installs} (${item.latest.percent}%), total=${item.installs}`);
                ...
                if (// 1. if latest version is older than two weeks
                    ...
                    // 3a. if the latest version is used by more than 10 percent of users
                    // 3b. or if latest version older 90 days and more than 5% of users
                    if ((item.latest.percent > 10) || 
                       ((item.latest.percent > 5) && (now.getTime() - latestTime.getTime()) > 90 * ONE_DAY)) {
