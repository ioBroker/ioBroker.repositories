...
async function checkIssues(latest, stable, stats, result) {
    for (const adapter in stable) {
        if (!adapter.startsWith('_') && adapter === 'hikvision-alarmserver' ) {
		...
		if (issue.title === newTitle) {
                    ...
                    if (matches && matches.length) {
                        ...
                        if (semver.gte( stable[adapter].version, issue.to)) {
			...
			} else if ( res.length && semver.gt( res[0].latest.version, issue.to))  {
                            console.log(`    adapter should be updated to ${res[0].latest.version} now`);
                            const comment = `This issue seems to be outdated\n\n` + 
                                `This issue suggests to update the stable version of this adapter to ${issue.to} but in meantime an update` + 
                                `to version ${res[0].latest.version} is sugested.\n\n` +
                                `So this issue will be closed and repleaced by an updates one.`;
                            try {
                                addComment(owner, adapter, issueId, comment);
                            } catch (e) {
                                console.log(`error adding comment to ${issueId}`);                          
...
console.log(`    Installs: stable=${item.stable.installs} (${item.stable.percent}%), latest=${item.latest.installs} (${item.latest.percent}%), total=${item.installs}`);
                ...
                if (// 1. if latest version is older than two weeks
                    ...
                    // 3a. if the latest version is used by more than 5 percent of users
                    // 3b. or if latest version older 30 days
