...
const objectStructure_1 = require("@iobroker/repochecker/lib/objectStructure");
// ─────────────────────────────────────────────────────────────────────────────
 ...
 * @param label
 */
async function safeAddLabel(prNumber, label) {
    console.log(`  Adding label "${label}"...`);
    try {
        await (0, common_1.addLabel)(prNumber, [label]);
 ...
 * @param label
 */
async function safeRemoveLabel(prNumber, label) {
    console.log(`  Removing label "${label}"...`);
    try {
        await (0, common_1.deleteLabel)(prNumber, label);
...
async function doIt() {
    ...
    const pr = await (0, common_1.getGithub)(`https://api.github.com/repos/ioBroker/ioBroker.repositories/pulls/${prNumber}`);
    const commentsRaw = await (0, common_1.getAllComments)(prNumber);
    ...
    const latestOurCommentTs = ourComments.length > 0 ? Math.max(...ourComments.map((c) => new Date(c.created_at).getTime())) : 0;
    ...
    if (!isRecheck && !isManual && latestOurCommentTs > latest.timestamp) {
        console.log(`A checkObjectStructure comment (${new Date(latestOurCommentTs).toISOString()}) already exists ` +
    ...
    // ── 7. Download ───────────────────────────────────────────────────────
    const tmpDir = fs_1.default.mkdtempSync(path_1.default.join(os_1.default.tmpdir(), 'checkObjectStructure-'));
    const tmpFile = path_1.default.join(tmpDir, latest.filename);
    ...
    let raw;
    try {
        raw = fs_1.default.readFileSync(tmpFile, 'utf8');
    }
    catch (e) {
    ...
    const result = (0, objectStructure_1.checkObjectStructure)(objects, adapter);
    ...
    await (0, common_1.addComment)(prNumber, commentBody);
    ...
    if (ourComments.length > 0) {
        ...
        for (const c of ourComments) {
            try {
                await (0, common_1.deleteComment)(prNumber, c.id);
