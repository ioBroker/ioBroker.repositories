...
async function getLatestRepo() {
    return await (0, common_1.getUrl)('https://download.iobroker.net/sources-dist-latest.json');
}
async function getStableRepo() {
    return await (0, common_1.getUrl)('https://download.iobroker.net/sources-dist.json');
}
// eslint-disable-next-line @typescript-eslint/no-unused-vars -- kept for parity with build.ts, currently unused here
async function getStats() {
    return await (0, common_1.getUrl)('https://www.iobroker.net/data/statistics.json');
}
async function checkNpm(adapter) {
    // console.log (`checking ${adapter}`);
    let response;
    try {
        response = await (0, axios_1.default)(`https://registry.npmjs.org/iobroker.${adapter}`);
    }
    catch (e) {
        console.log(`${e}`);
    ...
    if (!response?.data) {
        try {
            response = await (0, axios_1.default)(`https://www.npmjs.com/package/iobroker.${adapter}`);
        }
        catch (e) {
            console.log(`${e}`);
...
async function checkRepos(adapters) {
    const result = [];
    const count = Object.keys(adapters).length;
    let idx = 0;
    console.log('');
...
async function generateIssue(adapter) {
    let issues;
    try {
        issues = await (0, common_1.getGithub)(`https://api.github.com/repos/${adapter.owner}/ioBroker.${adapter.adapter}/issues`);
    }
    catch (e) {
    ...
    if (!issues.length) {
        ...
        console.log(`create issue for ioBroker.${adapter.adapter}`);
        try {
            await (0, common_1.createIssue)(adapter.owner, `iobroker.${adapter.adapter}`, { title, body });
        }
        catch (e) {
...
async function updateSummaryIssue(adapters) {
    ...
    issues = await (0, common_1.getGithub)(`https://api.github.com/repos/ioBroker/ioBroker.repositories/issues?state=open&page=1&per_page=100`);
    issues = issues.filter((i) => i.state === 'open' && i.title.includes(title));
    if (!issues.length) {
        ...
        try {
            await (0, common_1.createIssue)('iobroker', 'iobroker.repositories', { title, body });
        }
        catch (e) {
    ...
    try {
        let done = false;
        while (!done) {
            const gitComments = await (0, common_1.getAllComments)(issueId);
            const exists = gitComments.find((comment) => comment.body.includes('## ioBroker adapter checker'));
            if (exists) {
                ...
                await (0, common_1.deleteComment)(issueId, exists.id);
            }
            else {
                done = true;
            }
    ...
    console.log(`${body}`);
    try {
        await (0, common_1.addComment)(issueId, body);
    }
    catch (e) {
