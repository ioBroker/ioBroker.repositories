...
const repochecker_1 = __importDefault(require("@iobroker/repochecker"));
const TEXT_RECHECK = 'RE-CHECK!';
const TEXT_COMMENT_TITLE = '## Automated adapter checker';
const TEXT_MULTIPLE_REPOSITORIES = 'Please create seperate PRs for every adpater to add or update. This PR might be closed as it changes or adds more than one adapter.';
...
async function fetchJsonAtRef(filename, ref) {
    try {
        ...
        const meta = await (0, common_1.getGithub)(url);
...
async function isPublicOrgMember(org, username) {
    try {
        ...
        await (0, common_1.getGithub)(`https://api.github.com/orgs/${encodeURIComponent(org)}/public_members/${encodeURIComponent(username)}`);
        return true;
    }
    catch {
        return false;
...
async function hasMergedRecentPr(owner, adapter, username) {
    try {
        const prs = await (0, common_1.getGithub)(`https://api.github.com/repos/${owner}/${adapter}/pulls?state=closed&per_page=100&sort=updated&direction=desc`);
        const merged = (prs || [])
            .filter((pr) => pr?.merged_at && !isDependabotPr(pr))
            .slice(0, RECENT_PR_SCAN_LIMIT);
        for (const pr of merged) {
            try {
                const detail = await (0, common_1.getGithub)(`https://api.github.com/repos/${owner}/${adapter}/pulls/${pr.number}`);
...
async function verifyAuthorLegitimacy(owner, adapter, username) {
    ...
    try {
        const ownerInfo = await (0, common_1.getGithub)(`https://api.github.com/users/${encodeURIComponent(owner)}`);
...
async function detectAffectedAdapter(prID) {
    ...
    const pr = await (0, common_1.getGithub)(`https://api.github.com/repos/ioBroker/ioBroker.repositories/pulls/${prID}`);
    ...
    const compare = await (0, common_1.getGithub)(`https://api.github.com/repos/ioBroker/ioBroker.repositories/compare/${baseRef}...${headRef}`);
    ...
    const prFiles = await (0, common_1.getGithub)(`https://api.github.com/repos/ioBroker/ioBroker.repositories/pulls/${prID}/files`);
    const sourceFiles = prFiles.filter((f) => f.filename.startsWith('sources-dist')).map((f) => f.filename);
...
function triggerRepoCheck(owner, adapter) {
    ...
    const axiosInstance = axios_1.default.create(); // create a clean instance to avoid injection from other packages
    return axiosInstance
        .post(`https://api.github.com/repos/iobroker-bot-orga/check-tasks/dispatches`, { event_type: 'check-repository', client_payload: { url: url } }, {
        headers: {
...
async function doIt() {
    ...
    const files = await (0, common_1.getGithub)(`https://api.github.com/repos/ioBroker/ioBroker.repositories/pulls/${prID}/files`);
    ...
    if (links.length > 1) {
        ...
        try {
            await (0, common_1.addLabel)(prID, ['multiple repositories']);
        }
        catch (e) {
            console.error(`Cannot add label 'multiple repositories': ${e}`);
        }
        try {
            const gitComments = await (0, common_1.getAllComments)(prID);
            const exists = gitComments.find((comment) => comment.body.includes(TEXT_MULTIPLE_REPOSITORIES));
            if (!exists) {
                await (0, common_1.addComment)(prID, TEXT_MULTIPLE_REPOSITORIES);
            }
        }
        catch (e) {
    ...
    for (let i = 0; i < links.length; i++) {
        ...
        if (legitimacyReason) {
            console.log(`PR author ${prAuthor} verified as legitimate maintainer of ${owner}/${adapter}: ${legitimacyReason}`);
            verificationLines.push(`:white_check_mark: PR creator @${prAuthor} has been verified as legitimate maintainer of [${adapter}](${link}) because the user **${legitimacyReason}**.`);
        }
        else {
            console.log(`PR author ${prAuthor} could not be verified as maintainer of ${owner}/${adapter} - flagging 'maintainer ?'`);
            maintainerMissing = true;
            verificationLines.push(`:warning: PR creator @${prAuthor} could **not** be verified as legitimate maintainer of [${adapter}](${link}). The user needs to be validated manually.`);
        }
        try {
            const latestSVG = await (0, common_1.getUrl)(`http://iobroker.live/badges/${adapter.replace('ioBroker.', '')}-installed.svg`);
        ...
        try {
            const stableSVG = await (0, common_1.getUrl)(`http://iobroker.live/badges/${adapter.replace('ioBroker.', '')}-stable.svg`);
        ...
        if (data.context) {
            ...
            if (data.context.warnings?.filter((warn) => warn.startsWith('[W')).length) {
            ...
            if (data.context.warnings?.filter((warn) => warn.startsWith('[S')).length) {
        ...
        if (isStable) {
            const latest = await (0, common_1.getUrl)('https://download.iobroker.net/sources-dist-latest.json');
            const stable = await (0, common_1.getUrl)('https://download.iobroker.net/sources-dist.json');
            const statistic = await (0, common_1.getUrl)('https://www.iobroker.net/data/statistics.json');
            comments.push({ text: `\n`, noDecorate: true });
            comments.push({
            ...
            if (submittedRelease === latestRelease) {
                ...
                try {
                    const npmInfo = await (0, common_1.getUrl)(`https://registry.npmjs.org/iobroker.${adapterName.toLowerCase()}`);
            ...
            if (stable[adapterName]) {
                ...
                await (0, common_1.addLabel)(prID, ['new at STABLE']);
            }
            comments.push({ text: ``, noDecorate: true });
            if (submittedRelease !== latestRelease) {
                comments.push({
                    text: `:warning: This PR does **not** submit the current latest release (${latestRelease}) ` +
            ...
            if (!latest[adapterName]) {
                await (0, common_1.addLabel)(prID, ['new at LATEST']);
                const gitComments = await (0, common_1.getAllComments)(prID);
                const exists = gitComments.find((comment) => comment.body.includes('## ioBroker repository information about New at LATEST tagging'));
                ...
                if (!exists) {
                    ...
                    try {
                        ...
                        await (0, common_1.addComment)(prID, body);
                    }
                    catch (e) {
    ...
    if (!someChecked) {
        ...
        try {
            await (0, common_1.deleteLabel)(prID, 'auto-checked ✔');
            await (0, common_1.deleteLabel)(prID, 'auto-checked ❌');
        }
        catch {
        ...
        try {
            if (errorsFound) {
                ...
                await (0, common_1.addLabel)(prID, ['must be fixed', 'auto-checked ❌']);
            }
            else {
                console.log(`setting labels for PR ${prID} - no errors detected`);
                await (0, common_1.addLabel)(prID, ['auto-checked ✔']);
            }
        }
        catch (e) {
        ...
        if (maintainerMissing) {
            try {
                ...
                await (0, common_1.addLabel)(prID, ['maintainer ?']);
            }
            catch (e) {
    ...
    try {
        ...
        if (exists) {
            await (0, common_1.deleteComment)(prID, exists.id);
        }
        exists = gitComments.find((comment) => comment.body === TEXT_RECHECK);
        if (exists) {
            await (0, common_1.deleteComment)(prID, exists.id);
        }
        await (0, common_1.addComment)(prID, comment);
    }
    catch (e) {
