...
async function checkRepos(latest, stable, stats) {
    ...
    for (const adapter in latest) {
        ...
        if (adapter.startsWith('_')) {
            ...
            try {
                const json = await (0, common_1.getGithub)(`https://api.github.com/repos/${owner}/ioBroker.${adapter}`);
                if (json.archived) {
                    const item = {
                        adapter: adapter,
                        owner: owner,
...
async function generateIssue(adapter) {
    let issues;
    issues = await (0, common_1.getGithub)(`https://api.github.com/repos/ioBroker/ioBroker.repositories/issues`);
