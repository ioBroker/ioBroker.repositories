...
if (process.env.OWN_GITHUB_TOKEN) {
    axios_1.default.defaults.headers.common.Authorization = `Bearer ${process.env.OWN_GITHUB_TOKEN}`;
}
...
describe('Test Repository', () => {
    it('Test Repository: latest', done => {
        const text = node_fs_1.default.readFileSync(`${__dirname}/../../sources-dist.json`);
        try {
            latest = JSON.parse(text.toString());
        }
        catch (e) {
            node_assert_1.default.equal(e, null, 'Error parsing sources-dist.json');
        }
        done();
    });
    it('Test Repository: stable', done => {
        const text = node_fs_1.default.readFileSync(`${__dirname}/../../sources-dist-stable.json`);
        try {
            stable = JSON.parse(text.toString());
        }
        catch (e) {
            node_assert_1.default.equal(e, null, 'Error parsing sources-dist-stable.json');
        }
        done();
    });
    it('Check reserved names', done => {
        ...
        node_assert_1.default.ok(!id, `Found reserved name in stable: ${id}`);
        // check the latest names
        id = Object.keys(latest).find(id => reservedAdapterNames.includes(id.replace('iobroker.', '').replace('ioBroker.', '')));
        node_assert_1.default.ok(!id, `Found reserved name in latest: ${id}`);
        done();
    });
    it('Test Repository: compare types', async () => {
        ...
        for (const id in stable) {
            if (Object.prototype.hasOwnProperty.call(stable, id) && id !== '_repoInfo') {
                node_assert_1.default.equal(id, id.toLowerCase(), `Adapter id ${id} is not lowercase`);
                node_assert_1.default.notEqual(latest[id], undefined, `${id} not in latest but in stable`);
                node_assert_1.default.notEqual(latest[id].type, undefined, `${id} missing type in latest`);
                node_assert_1.default.notEqual(latest[id].type, '', `${id} has empty type in latest`);
                node_assert_1.default.equal(latest[id].type, stable[id].type, `${id} type mismatch: latest(${latest[id].type}) vs stable(${stable[id].type})`);
            }
        }
...
*/
            node_assert_1.default.ok(latest[name].meta, `${name} missing meta in latest`);
            node_assert_1.default.ok(!latest[name].meta.match(/\s/), `${name} meta has spaces in latest`);
            if (name !== 'js-controller') {
                if (!latest[name].icon) {
                    console.error(`Adapter ${name} has no icon in latest`);
                }
                node_assert_1.default.ok(latest[name].icon, `${name} missing icon in latest`);
                node_assert_1.default.ok(!latest[name].icon.match(/\s/), `${name} icon has spaces in latest`);
            }
        }
        done();
    });
...
*/
            ...
            node_assert_1.default.ok(stable[name].version, `${name} missing version in stable`);
            node_assert_1.default.ok(!stable[name].version.match(/\s/), `${name} version has spaces in stable`);
            node_assert_1.default.ok(stable[name].meta, `${name} missing meta in stable`);
            node_assert_1.default.ok(!stable[name].meta.match(/\s/), `${name} meta has spaces in stable`);
            if (name !== 'js-controller') {
                if (!stable[name].icon) {
                    console.error(`Adapter ${name} has no icon in stable`);
                }
                node_assert_1.default.ok(stable[name].icon, `${name} missing icon in stable`);
                node_assert_1.default.ok(!stable[name].icon.match(/\s/), `${name} icon has spaces in stable`);
            }
        }
        done();
    });
    it('Test Repository: Compare stable and latest', done => {
        ...
        for (const name in stable) {
            ...
            node_assert_1.default.ok(latest[name], `${name} is in stable but not in latest`);
            node_assert_1.default.strictEqual(latest[name].meta, stable[name].meta, `${name} meta mismatch stable vs latest`);
            node_assert_1.default.strictEqual(latest[name].icon, stable[name].icon, `${name} icon mismatch stable vs latest`);
            node_assert_1.default.strictEqual(latest[name].type, stable[name].type, `${name} type mismatch stable vs latest`);
        }
        done();
    });
