...
async function doIt() {
    ...
    let prs;
    do {
        prs = await (0, common_1.getGithub)(`https://api.github.com/repos/ioBroker/ioBroker.repositories/pulls?state=open&per_page=100&page=${page}`);
    ...
    const targetPRs = allPRs.filter(pr => pr.labels &&
        pr.labels.some((label) => label.name.toUpperCase().includes('LATEST') || label.name.toUpperCase().includes('STABLE')));
    ...
    for (const pr of targetPRs) {
        ...
        try {
            await (0, common_1.addComment)(pr.number, TEXT_RECHECK);
