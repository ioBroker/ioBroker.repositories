...
function getPullRequestNumber() {
    ...
    if (process.env.GITHUB_EVENT_PATH) {
        const event = JSON.parse(node_fs_1.default.readFileSync(process.env.GITHUB_EVENT_PATH, 'utf8'));
        return event.pull_request ? event.pull_request.number : event.issue ? event.issue.number : '';
    }
    return '';
}
async function cleanupLabels() {
    const labels = await (0, common_1.getLabels)('');
    labels.forEach((l) => {
        ...
        issue.labels.forEach((l) => {
            ...
            if (result) {
                if (usedLabels.includes(`${issue.number}-${l.name}`)) {
                    ...
                    (0, common_1.deleteLabel)(issue.number, l.name);
                }
            }
        });
    }
...
async function addLabelToIssue(label, issue) {
    let labels = await (0, common_1.getLabels)('');
    labels = labels.filter((f) => {
        return f.name === `${label}`;
    });
    if (!labels.length) {
        console.log(`    will create label ${label}`);
        await (0, common_1.createLabel)(`${label}`, `remind after ${label}`, `ffffff`);
    }
    await addUsedLabels(issue.number, label);
    await (0, common_1.addLabel)(issue.number, [`${label}`]);
}
async function handleBrandNew(pIssues) {
    for (const issue of pIssues) {
        if (issue.labels.find((label) => label.name === 'STABLE - brand new')) {
            ...
            const comments = await (0, common_1.getAllComments)(issue.number);
            let found = false;
            let comment;
            ...
            if (comment) {
                ...
                if (result) {
                    let targetTs = new Date(Number(result[3]), Number(result[2]) - 1, Number(result[1]), 0, 0, 0).getTime();
                    ...
                    if (nowTs < targetTs) {
                        ...
                        await (0, common_1.updateLabel)(`${label}`, `remind after ${dateStr}`, `ffffff`);
                    }
                    else {
                        console.log(`    should be merged now (deadline ${dateStr})`);
                        await (0, common_1.updateLabel)(`${label}`, `remind after ${dateStr}`, `ff0000`);
                        await (0, common_1.addLabel)(issue.number, ['⚠️check']);
                    }
                }
                found = true;
            }
...
async function handleOthers(pIssues) {
    for (const issue of pIssues) {
        if (!issue.labels.find((label) => label.name === 'STABLE - brand new')) {
            console.log(`checking PR ${issue.number}`);
            const comments = await (0, common_1.getAllComments)(issue.number);
            const comment = comments.findLast((c) => /reminder (\d+\.\d+\.\d+|none)/g.exec(c.body));
...
async function doIt() {
    // read all issues
    let issues = await (0, common_1.getGithub)(`https://api.github.com/repos/iobroker/ioBroker.repositories/issues?per_page=100`);
