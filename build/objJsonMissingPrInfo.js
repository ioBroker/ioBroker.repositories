...
async function checkLabel(prID, label) {
    const lbls = await (0, common_1.getLabels)(prID);
...
async function doIt() {
    ...
    const exists = gitComments.find((comment) => comment.body.includes('### object dump is missing'));
    ...
    if (!exists && labelIsSet) {
        let body = `A German translation is provided below. / Die deutsche Übersetzung ist weiter unten zu finden.\n\n`;
        ...
        try {
            console.log(`triggering handleReminder workflow`);
            await (0, common_1.triggerWorkflow)('handleReminder.yml', 'master');
        }
        catch (e) {
            console.error(`warning: cannot trigger handleReminder workflow:`);
            console.log(`           ${e}`);
        }
    }
    return 'done';
