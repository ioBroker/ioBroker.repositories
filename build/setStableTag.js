...
async function getStableRepo() {
    return await (0, common_1.getUrl)('https://raw.githubusercontent.com/ioBroker/ioBroker.repositories/master/sources-dist-stable.json');
}
async function getNpmMeta(pAdapter) {
    return await (0, common_1.getUrl)(`https://registry.npmjs.org/iobroker.${pAdapter}`);
}
    ...
    for (const adapter in stable) {
        if (ADAPTER_LIST.includes(adapter)) {
            ...
            if (repoRelease === npmRelease) {
                ...
                try {
                    const result = (0, node_child_process_1.execSync)(cmd, { encoding: 'utf-8' });
                    console.log(result);
                }
                catch {
