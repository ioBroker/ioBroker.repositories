...
for (const [name, entry] of Object.entries(stable)) {
    ...
    if (typeof attributes.version === 'string') {
        ...
        logCheck(name, 'stableVersion', { version: attributes.version }, hasValidVersion);
        ...
        logCheck(name, 'stableVersion', { version: attributes.version }, false);
    }

    const latestEntry = latest[name] as Record<string, unknown> | undefined;
    ...
    for (const attribute of ['meta', 'icon', 'type']) {
        ...
        logCheck(
            ...
            { attribute, latest: latestEntry[attribute], stable: attributes[attribute] },
            isEqual,
        );
