...
import { addComment, getGithub, getUrl, createIssue, deleteComment, getAllComments } from './common';
...
async function checkNpm(adapter: string) {
    ...
    let ret = '';
    if (!body.maintainers.find((user: any) => user.name === 'bluefox' || user.name === 'iobluefox')) {
...
async function mergeRepos(latest: any, stable: any) {
    const adapters = [];
...
async function checkRepos(adapters: any) {
    const result = [];
...
async function generateIssue(adapter: any) {
    let issues;
    try {
    ...
    issues = issues.filter((i: any) => i.state === 'open' && i.title.includes(title));
    if (!issues.length) {
...
async function updateSummaryIssue(adapters: any) {
    ...
    try {
        ...
        while (!done) {
            ...
            const exists = gitComments.find((comment: any) => comment.body.includes('## ioBroker adapter checker'));
            if (exists) {
                let body = exists.body.split('\n');
                body = body.filter((l: string) => l.startsWith('- [ ]'));
                body.forEach((a: string) => !oldList.includes(a) && oldList.push(a));
