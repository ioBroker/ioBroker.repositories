...
function findPath(path: string, url: string): string {
    ...
    if (url[0] === '/') {
        return `${import.meta.dirname}/..${url}`;
    }
    return `${import.meta.dirname}/../${path}${url}`;
}
...
function getJson(urlOrPath: string, callback?: (...args: any[]) => void): void {
    ...
    if (urlOrPath && typeof urlOrPath === 'object') {
        ...
        if (
            ...
            } else if (fs.existsSync(`${import.meta.dirname}/../${urlOrPath}`)) {
                try {
                    sources = JSON.parse(fs.readFileSync(`${import.meta.dirname}/../${urlOrPath}`).toString());
                } catch (e) {
                    console.log(`Cannot parse json file from ${import.meta.dirname}/../${urlOrPath}. Error: ${e}`);
                    callback?.(null, urlOrPath);
                    return;
                }
                callback?.(sources, urlOrPath);
            } else if (fs.existsSync(`${import.meta.dirname}/../tmp/${urlOrPath}`)) {
                try {
                    sources = JSON.parse(fs.readFileSync(`${import.meta.dirname}/../tmp/${urlOrPath}`).toString());
                } catch (e) {
                    console.log(`Cannot parse json file from ${import.meta.dirname}/../tmp/${urlOrPath}. Error: ${e}`);
                    callback?.(null, urlOrPath);
                    return;
                }
                callback?.(sources, urlOrPath);
...
export function getRepositoryFile(
    ...
    if (urlOrPath && typeof urlOrPath === 'object') {
        ...
        try {
            const sourcesDist = JSON.parse(
                fs.readFileSync(`${import.meta.dirname}/../conf/sources-dist.json`).toString(),
            );
...
function getDefaultDataDir(): string {
    //var dataDir = import.meta.dirname.replace(/\\/g, '/');
    //dataDir = dataDir.split('/');

    // If installed with npm
    if (fs.existsSync(`${import.meta.dirname}/../../../node_modules/iobroker.js-controller`)) {
