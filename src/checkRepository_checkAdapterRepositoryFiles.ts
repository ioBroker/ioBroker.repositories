import { getRepositoryEntries, loadRepository, logAdapterStart, logCheck, logDownload } from './checkRepository_common';
import type { LogOptions, RepoEntry, Repository } from './types';

/** Injectable so the unit tests can run the check without touching the network. */
export interface CheckAdapterRepositoryFilesOptions {
    getJson?: (url: string) => Promise<any>;
    getUrl?: (url: string) => Promise<void>;
    concurrency?: number;
    logOptions?: LogOptions;
}

export interface CheckAdapterRepositoryFilesResult {
    errors: string[];
    failingAdapters: string[];
}

async function fetchJson(url: string): Promise<any> {
    const response = await fetch(url, { signal: AbortSignal.timeout(30000) });
    if (!response.ok) {
        throw new Error(`HTTP ${response.status}`);
    }
    return response.json();
}

async function fetchUrl(url: string): Promise<void> {
    const response = await fetch(url, {
        method: 'GET',
        redirect: 'follow',
        signal: AbortSignal.timeout(30000),
    });
    if (!response.ok) {
        throw new Error(`HTTP ${response.status}`);
    }
    await response.arrayBuffer();
}

async function mapWithConcurrency<T>(
    entries: T[],
    concurrency: number,
    worker: (entry: T, index: number) => Promise<void>,
): Promise<void> {
    let index = 0;

    async function runNext(): Promise<void> {
        if (index >= entries.length) {
            return;
        }

        const currentIndex = index;
        index++;
        await worker(entries[currentIndex], currentIndex);
        await runNext();
    }

    const workers: Promise<void>[] = [];
    const limit = Math.min(concurrency, entries.length);
    for (let i = 0; i < limit; i++) {
        workers.push(runNext());
    }
    await Promise.all(workers);
}

export async function checkAdapterRepositoryFiles(
    repository: Repository,
    options: CheckAdapterRepositoryFilesOptions = {},
): Promise<CheckAdapterRepositoryFilesResult> {
    const { getJson = fetchJson, getUrl = fetchUrl, concurrency = 10, logOptions } = options;
    const entries = getRepositoryEntries(repository);
    const errors: string[] = [];
    const failingAdapters = new Set<string>();
    const metaCache = new Map<string, Promise<any>>();
    const iconCache = new Map<string, Promise<void>>();

    await mapWithConcurrency(entries, concurrency, async ([name, entry]: [string, RepoEntry]) => {
        logAdapterStart(name, logOptions);

        try {
            if (!metaCache.has(entry.meta)) {
                metaCache.set(
                    entry.meta,
                    (async () => {
                        try {
                            const response = await getJson(entry.meta);
                            logDownload(entry.meta, true, logOptions, { adapter: name });
                            return response;
                        } catch (error) {
                            logDownload(entry.meta, false, logOptions, { adapter: name, error: error.message });
                            throw error;
                        }
                    })(),
                );
            }
            const ioPackage = await metaCache.get(entry.meta);
            logCheck(name, 'metaDownload', { url: entry.meta }, true, logOptions);

            const matchesName = ioPackage?.common?.name === name;
            logCheck(
                name,
                'metaCommonName',
                { expected: name, actual: ioPackage?.common?.name },
                matchesName,
                logOptions,
            );

            if (!matchesName) {
                errors.push(`Adapter "${name}" common.name mismatch in io-package.json: "${ioPackage?.common?.name}"`);
                failingAdapters.add(name);
            }

            const matchesType = ioPackage?.common?.type === entry.type;
            logCheck(
                name,
                'metaCommonType',
                { expected: entry.type, actual: ioPackage?.common?.type },
                matchesType,
                logOptions,
            );

            if (!matchesType) {
                errors.push(
                    `Adapter "${name}" common.type mismatch: repository="${entry.type}" io-package.json="${ioPackage?.common?.type}"`,
                );
                failingAdapters.add(name);
            }
        } catch (error) {
            logCheck(name, 'metaDownload', { url: entry.meta, error: error.message }, false, logOptions);
            errors.push(`Adapter "${name}" meta "${entry.meta}" could not be fetched: ${error.message}`);
            failingAdapters.add(name);
        }

        try {
            if (!iconCache.has(entry.icon)) {
                iconCache.set(
                    entry.icon,
                    (async () => {
                        try {
                            await getUrl(entry.icon);
                            logDownload(entry.icon, true, logOptions, { adapter: name });
                        } catch (error) {
                            logDownload(entry.icon, false, logOptions, { adapter: name, error: error.message });
                            throw error;
                        }
                    })(),
                );
            }
            await iconCache.get(entry.icon);
            logCheck(name, 'iconDownload', { url: entry.icon }, true, logOptions);
        } catch (error) {
            logCheck(name, 'iconDownload', { url: entry.icon, error: error.message }, false, logOptions);
            errors.push(`Adapter "${name}" icon "${entry.icon}" could not be fetched: ${error.message}`);
            failingAdapters.add(name);
        }
    });

    return {
        errors,
        failingAdapters: [...failingAdapters],
    };
}

async function run(): Promise<void> {
    const latest = loadRepository('sources-dist.json');
    const result = await checkAdapterRepositoryFiles(latest);

    for (const error of result.errors) {
        console.error(error);
    }

    if (result.errors.length) {
        console.error(`Adapters with repository file errors: ${result.failingAdapters.join(', ')}`);
        process.exit(1);
    }

    console.log('sources-dist.json: repository file checks passed.');
}

if (require.main === module) {
    run().catch(error => {
        console.error(error);
        process.exit(1);
    });
}
