 ...
 *   5. Downloads the latest JSON file and logs its content (max 50 lines).
 ...
 *   8. Label logic:
 *        • Issues found  → add "objects ❌", add "must be fixed", remove "objects ✔"
 *        • No issues     → add "objects ✔",  remove "objects ❌"
 *        "must be fixed" is never removed by this script.
 */
...
import { addComment, addLabel, deleteComment, deleteLabel, getAllComments, getGithub } from './common';
import { checkObjectStructure } from '@iobroker/repochecker/lib/objectStructure';
...
// Labels managed by this script.
// "must be fixed" is only ever added, never removed.
const LABEL_MUST_FIX = 'must be fixed'; // added when issues are found; never removed
const LABEL_PASS = 'objects ✔'; // added on clean pass;    removed when issues are found
const LABEL_FAIL = 'objects ❌'; // added when issues found; removed on clean pass

// Maximum number of lines of the downloaded JSON file printed to the Actions log.
...
//
const ERROR_EXPLANATIONS: Record<string, { en: string; de: string }> = {
    // ── E1xxx  State common-block validation ─────────────────────────────

    E1001: {
        en:
            'The `common` property of a state object must be a plain JSON object `{}`.' +
        ...
        de:
            'Die `common`-Eigenschaft eines State-Objekts muss ein einfaches JSON-Objekt `{}` sein.' +
    ...
    E1002: {
        en:
            'The `common` block contains one or more keys that are not defined in the ioBroker state schema.' +
        ...
        de:
            'Der `common`-Block enthält einen oder mehrere Schlüssel, die im ioBroker-State-Schema nicht definiert sind.' +
    ...
    E1003: {
        en:
            'A required key is missing from the `common` block of the state.' +
        ...
        de:
            'Im `common`-Block des States fehlt ein erforderlicher Schlüssel.' +
    ...
    E1004: {
        en:
            'A key in the `common` block has a value of the wrong JavaScript type.' +
        ...
        de:
            'Ein Schlüssel im `common`-Block hat einen Wert des falschen JavaScript-Typs.' +
    ...
    E1005: {
        en:
            'A key that is only valid for specific values of `common.type` has been used with an incompatible type.' +
        ...
        de:
            'Ein Schlüssel, der nur für bestimmte Werte von `common.type` gültig ist, wurde mit einem inkompatiblen Typ verwendet.' +
    ...
    E1006: {
        en:
            'The value of a `common` key does not match the declared `common.type` of the state.' +
        ...
        de:
            'Der Wert eines `common`-Schlüssels stimmt nicht mit dem deklarierten `common.type` des States überein.' +
    ...
    E1007: {
        en:
            '`common.name` must be either a plain string or an i18n object whose values are all strings' +
        ...
        de:
            '`common.name` muss entweder eine einfache Zeichenkette oder ein i18n-Objekt sein, dessen Werte allesamt Zeichenketten sind' +
    ...
    E1008: {
        en:
            'The value assigned to `common.role` is not listed in the ioBroker role catalogue.' +
            ' See https://github.com/ioBroker/ioBroker.docs/blob/master/docs/en/dev/stateroles.md for a description of valid roles.' +
        ...
        de:
            'Der in `common.role` angegebene Wert ist nicht im ioBroker-Rollenkatalog aufgeführt.' +
            ' Eine Beschreibung der Rollen findet sich hier: https://github.com/ioBroker/ioBroker.docs/blob/master/docs/en/dev/stateroles.md.' +
    ...
    E1009: {
        en:
            'The chosen `common.role` does not support the `common.type` that has been declared for this state.' +
            ' See https://github.com/ioBroker/ioBroker.docs/blob/master/docs/en/dev/stateroles.md for a description of valid roles.' +
        ...
        de:
            'Die gewählte `common.role` unterstützt den für diesen State deklarierten `common.type` nicht.' +
            ' Eine Beschreibung der Rollen findet sich hier: https://github.com/ioBroker/ioBroker.docs/blob/master/docs/en/dev/stateroles.md.' +
    ...
    E1010: {
        en:
            'The `common.read` flag does not match the value required by the assigned role.' +
            ' See https://github.com/ioBroker/ioBroker.docs/blob/master/docs/en/dev/stateroles.md for a description of valid roles.' +
        ...
        de:
            'Das Flag `common.read` stimmt nicht mit dem von der zugewiesenen Rolle geforderten Wert überein.' +
            ' Eine Beschreibung der Rollen findet sich hier: https://github.com/ioBroker/ioBroker.docs/blob/master/docs/en/dev/stateroles.md.' +
    ...
    E1011: {
        en:
            'The `common.write` flag does not match the value required by the assigned role.' +
            ' See https://github.com/ioBroker/ioBroker.docs/blob/master/docs/en/dev/stateroles.md for a description of valid roles.' +
            ...
            ' the intended read/write semantics of your state.',
        de:
            'Das Flag `common.write` stimmt nicht mit dem von der zugewiesenen Rolle geforderten Wert überein.' +
            ' Eine Beschreibung der Rollen findet sich hier: https://github.com/ioBroker/ioBroker.docs/blob/master/docs/en/dev/stateroles.md.' +
    ...
    E2001: {
        en:
            'An object hierarchy mixes hierarchy-typed objects (device, channel, folder) with non-hierarchy' +
        ...
        de:
            'Eine Objekthierarchie mischt Objekte vom Hierarchietyp (device, channel, folder) mit Nicht-Hierarchieobjekten' +
    ...
    E2002: {
        en:
            'More than one `device` object appears along the same object id chain.' +
        ...
        de:
            'Entlang desselben Objekt-ID-Pfads tritt mehr als ein `device`-Objekt auf.' +
    ...
    E2003: {
        en:
            'A `device` object appears after a `channel` object in the same hierarchy chain.' +
        ...
        de:
            'Ein `device`-Objekt erscheint nach einem `channel`-Objekt im selben Hierarchiepfad.' +
    ...
    E2004: {
        en:
            'A `state` object has child objects beneath it in the id tree.' +
        ...
        de:
            'Ein `state`-Objekt hat untergeordnete Objekte in der ID-Hierarchie.' +
    ...
    E3001: {
        en:
            'The uploaded file does not contain a plain JSON object at the root level.' +
        ...
        de:
            'Die hochgeladene Datei enthält auf der obersten Ebene kein einfaches JSON-Objekt.' +
    ...
    E3002: {
        en:
            'A root-level entry in the object dump is invalid: either its value is not a plain object,' +
        ...
        de:
            'Ein Eintrag auf der obersten Ebene des Objekt-Dumps ist ungültig: Entweder ist sein Wert kein einfaches Objekt,' +
    ...
    E3003: {
        en:
            'Due to earlier structural errors (see E3002), the file cannot be interpreted as a valid' +
        ...
        de:
            'Aufgrund früherer Strukturfehler (siehe E3002) kann die Datei nicht als gültiger' +
    ...
    E3004: {
        en:
            'A required top-level key is missing from an ioBroker object record.' +
        ...
        de:
            'Ein erforderlicher Schlüssel fehlt auf der obersten Ebene eines ioBroker-Objektdatensatzes.' +
    ...
    E3005: {
        en:
            'A top-level key of an ioBroker object record has the wrong JavaScript type, or a key that is' +
        ...
        de:
            'Ein Schlüssel auf der obersten Ebene eines ioBroker-Objektdatensatzes hat den falschen JavaScript-Typ,' +
    ...
    E3006: {
        en:
            'An ioBroker object record contains one or more keys at the root level that are not part of' +
        ...
        de:
            'Ein ioBroker-Objektdatensatz enthält auf der obersten Ebene einen oder mehrere Schlüssel,' +
    ...
    E3007: {
        en:
            'The `type` field of an ioBroker object is set to a value that is not in the list of allowed object types.' +
        ...
        de:
            'Das `type`-Feld eines ioBroker-Objekts ist auf einen Wert gesetzt, der nicht in der Liste der erlaubten Objekttypen steht.' +
    ...
    E3008: {
        en:
            'The object id contains characters that are forbidden in ioBroker object ids.' +
        ...
        de:
            'Die Objekt-ID enthält Zeichen, die in ioBroker-Objekt-IDs nicht erlaubt sind.' +
    ...
    E3009: {
        en:
            'An intermediate parent object is missing from the object dump.' +
        ...
        de:
            'Ein zwischengelagertes Elternobjekt fehlt im Objekt-Dump.' +
    ...
    E3010: {
        en:
            'A state object does not define `common.type`.' +
        ...
        de:
            'Ein State-Objekt definiert `common.type` nicht.' +
    ...
    E3011: {
        en:
            'The value of `common.type` is not one of the recognised ioBroker state data types.' +
            ' Valid values are: `number`, `string`, `boolean`, `array`, `object`, `mixed`, `file`, `json`.\n\n' +
            '**Fix:** Change `common.type` to one of these valid values.',
        de:
            'Der Wert von `common.type` ist keiner der bekannten ioBroker-State-Datentypen.' +
    ...
    W1001: {
        en:
            'An i18n object (e.g. `common.name` or `common.desc`) is missing one or more *recommended*' +
        ...
        de:
            'Ein i18n-Objekt (z. B. `common.name` oder `common.desc`) enthält nicht alle *empfohlenen*' +
    ...
    W3001: {
        en:
            'The object id contains characters outside the recommended set `[a-zA-Z0-9_,-]`.' +
        ...
        de:
            'Die Objekt-ID enthält Zeichen außerhalb des empfohlenen Zeichensatzes `[a-zA-Z0-9_,-]`.' +
 ...
 * @param comment
 */
function isOurComment(comment: any) {
 ...
 *
 * @param text  Raw Markdown body.
 * @returns  In document order, no duplicates.
 */
function extractJsonAttachments(text: string) {
    if (!text) {
        return [];
    }

    const UPLOAD_REGEX = /\[([^\]]*)\]\((https:\/\/github\.com\/user-attachments\/files\/\d+\/([^\s)]+\.json))\)/gi;
...
async function downloadFile(url: string, destPath: string) {
    const response = await axios({
        ...
        headers: {
            Authorization: process.env.OWN_GITHUB_TOKEN ? `token ${process.env.OWN_GITHUB_TOKEN}` : undefined,
...
/**
 * Attach a label to the PR.  Non-fatal: logs a warning on failure.
 *
 * @param prNumber
 * @param label
 */
async function safeAddLabel(prNumber: string | number, label: string) {
    console.log(`  Adding label "${label}"...`);
    try {
        await addLabel(prNumber, [label]);
        console.log(`  Label "${label}" added.`);
    } catch (e) {
        console.warn(`  Failed to add label "${label}": ${e.message || e}`);
        console.warn('  Make sure the label exists in the repository settings.');
    }
}

/**
 * Remove a label from the PR.
 * A 404 response (label not present on the PR) is silently ignored.
 * All other errors are logged as warnings but never thrown.
 *
 * @param prNumber
 * @param label
 */
async function safeRemoveLabel(prNumber: string | number, label: string) {
    ...
    try {
        ...
        if (status === 404) {
            ...
            console.warn(`  Failed to remove label "${label}": ${e.message || e}`);
        }
    }
}
 ...
 *
 * @param issues  Combined errors + warnings array.
 * @returns  Markdown, or empty string when nothing to explain.
 */
function buildExplanationSection(issues: any) {
    if (issues.length === 0) {
        return '';
    }
 ...
 */
function formatComment(result: any, filename: string) {
    const lines = [];
    ...
    lines.push('>');
    lines.push(
        '>This comment contains information in English and German. / Dieser Kommentar enthält Informationen auf Englisch und Deutsch.',
    );
    ...
    lines.push('');
    lines.push('');
    lines.push('Please fix issues detcted **and attach a new object dump afterwards**');
    lines.push('');
 ...
 */
function adapterNameFromFilename(filename: string) {
    return filename
...
async function doIt() {
    ...
    if (process.env.GITHUB_EVENT_PATH) {
        ...
        isRecheck =
            ...
            event.comment &&
            event.comment.body &&
            event.comment.body.trim() === 'RE-CHECK!';
    }
    ...
    const pr = await getGithub(`https://api.github.com/repos/ioBroker/ioBroker.repositories/pulls/${prNumber}`);
    ...
    const latestOurCommentTs =
        ourComments.length > 0 ? Math.max(...ourComments.map((c: any) => new Date(c.created_at).getTime())) : 0;
    ...
    // ── 6. Skip if already checked and not a manual/re-check run ──────────
    ...
    const truncated = rawLines.length > LOG_MAX_LINES;
    const logLines = truncated ? rawLines.slice(0, LOG_MAX_LINES) : rawLines;

    console.log(
        `\n──── Downloaded file content: ${latest.filename}` +
            ` (${rawLines.length} line(s) total${truncated ? `, showing first ${LOG_MAX_LINES}` : ''}) ────`,
    );
    console.log(logLines.join('\n'));
    if (truncated) {
        console.log(`... [${rawLines.length - LOG_MAX_LINES} more line(s) not shown]`);
    }
    ...
    //     • Add    "objects ❌"   (check failed)
    //     • Add    "must be fixed" (permanent flag – never removed by this script)
    //     • Remove "objects ✔"   (no longer passing)
    //
    //   No issues:
    //     • Add    "objects ✔"   (clean pass)
    //     • Remove "objects ❌"   (no longer failing)
    //     "must be fixed" is intentionally left untouched on a clean pass.
    //
    const hasIssues = result.errors.length > 0 || result.warnings.length > 0;
    console.log(`\nUpdating labels on PR #${prNumber} (hasIssues=${hasIssues})...`);

    if (hasIssues) {
        await safeAddLabel(prNumber, LABEL_FAIL);
        await safeAddLabel(prNumber, LABEL_MUST_FIX);
        await safeRemoveLabel(prNumber, LABEL_PASS);
    } else {
        await safeAddLabel(prNumber, LABEL_PASS);
        await safeRemoveLabel(prNumber, LABEL_FAIL);
        // LABEL_MUST_FIX is deliberately never removed.
    }
