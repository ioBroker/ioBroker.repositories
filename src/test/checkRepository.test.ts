...
import { findReservedAdapterNames } from '../checkRepository_checkAdapterNames';
import { checkAdapterRepositoryFiles } from '../checkRepository_checkAdapterRepositoryFiles';
    ...
    it('checks all adapter repository files before failing', async () => {
        ...
        const result = await checkAdapterRepositoryFiles(
            ...
            },
            {
                ...
                logOptions: {
                    enabled: true,
                    logger: (message: string) => started.push(message),
        ...
        const startLogs = started.filter((line: string) => line.includes('Checking adapter')).sort();
        ...
        assert.ok(
            started.some((line: string) => line.includes('download url="https://example.com/valid/io-package.json"')),
        );
        assert.ok(
            started.some((line: string) => line.includes('download url="https://example.com/broken-icon/icon.png"')),
        );
        assert.ok(started.some((line: string) => line.includes('adapter="brokenName" check="metaCommonName"')));
        assert.ok(started.some((line: string) => line.includes('adapter="brokenIcon" check="iconDownload"')));
    });
