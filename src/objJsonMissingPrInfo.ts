...
import { addComment, deleteComment, getLabels, getAllComments, triggerWorkflow } from './common';
...
async function doIt() {
    ...
    const exists = gitComments.find((comment: any) => comment.body.includes('### object dump is missing'));
    ...
    if (!exists && labelIsSet) {
        ...
        try {
            ...
            await triggerWorkflow('handleReminder.yml', 'master');
        } catch (e) {
