...
* Consider and understand the asynchronous nature of JavaScript and make sure to know what will happen in parallel and what makes more sense to be sequentially! It is ok to use `callbacks` or `Promises/async/await` - the later makes it easier to understand and control how your code really flows.
...
* If an adapter open a port or bind socket to some IP-address, the attributes must be called `port` and `bind` (`v6bind` for IPv6).
