...
* a Scheduling library just to implement intervals is overkill :-) setInterval/setTimeout should be completely sufficiant AND has the good side effe t that requests are not done all at the same time, but start when the adapter starts.
