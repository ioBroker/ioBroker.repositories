...
* Respect Object and state definitions, types and roles. Values not defined here must not be used. Discussions about missing roles or types are welcome:
...
* use jsonConfig for adminUI unless there is a special need to use REACT. Use i18n file based translation if possible.
* Do not use an adminTab unless functionality cannot be provided otherwise. Discuss with core team (@Apollon77) before implementing it to avoid later review discussions.
...
* Do not use `process.exit()` because this breaks compact mode. Use `adapter.terminate()`.
...
* Consider using ESLint or other JavaScript code and type checker to see errors in your code before releasing a new version. Use @iobroker/eslint-config to setup lint and prettier.
