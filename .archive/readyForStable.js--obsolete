    ...
    for (const adapter in latest) {
	    ...
	    //console.log (`\n[DEBUG] checking ${latest[adapter].meta}\n`);
            ...
            for (const issue of issues) {
				...
				// ########SCRIPT IS END OF LOIFE - closing all issues #################
                console.log(`    ${issue.title} detected - issue ${issueId} - will close issue due to cleanup`);
				try {
					await closeIssue(owner, adapter, issueId);
				} catch (e) {
					console.log(e);
				}
				continue;
